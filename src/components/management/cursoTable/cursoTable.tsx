...
export default function CursoTable({
  ...
  setKeywordCursos: Dispatch<SetStateAction<string>>;
  keywordCursos: string;
  fetchingCursos: boolean;
  addCurso: () => void;
  cursoInput: string;
  setCursoInput: Dispatch<SetStateAction<string>>;
  cursosArr: CursoWrap[];
  setCursoDeleteIndex: Dispatch<SetStateAction<Curso | null>>;
  handleCursoEdit: (index: number) => void;
  editCurso: (id: string, newName: string) => Promise<void>;
}) {
  return (
    <>
                  ...
                  <input
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && curso._id) {
                        const inputElmt = (
                          document.getElementById(curso._id) as HTMLInputElement
                        ).value;
