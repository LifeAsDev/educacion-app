...
export default function Management() {
  ...
  >("usuarios");
  const [cursosArr, setCursosArr] = useState<Curso[]>([]);
  ...
  const [cursosArrFilter, setCursosArrFilter] = useState<Curso[]>([]);
  const [cursoDeleteIndex, setCursoDeleteIndex] = useState<null | number>(null);
  ...
  const addCurso = () => {
    if (
      ...
      const newCurso: Curso = { name: cursoInput.trim() };
      setCursosArr((prev) => [...prev, newCurso]);
      setCursoInput("");

      const fetchSubmit = async (newCurso: Curso) => {
        try {
          ...
          if (res.ok) {
            const newCursos = [...cursosArr];
            newCursos[actualIndex] = {
              ...newCurso,
              _id: resData.id,
            };
      ...
      fetchSubmit(newCurso);
    }
  };

  const deleteCurso = async () => {
    const newCursosArr = [...cursosArr];
    const deleteUser = newCursosArr.splice(cursoDeleteIndex as number, 1);
  ...
  useEffect(() => {
    ...
    const filteredCursos = cursosArr.filter((curso) =>
      curso.name.toLowerCase().includes(keywordCursos.toLowerCase())
    );

    setCursosArrFilter(filteredCursos);
  }, [cursosArr, keywordCursos]);
        ...
        <div className={styles.deleteUserBox}>
          <div className={styles.deleteUserModal}>
            <p>
              ¿Estás seguro que deseas borrar
              <br /> el curso
              <span>{` ${cursosArr[cursoDeleteIndex].name}`}</span>?
            </p>
            <div className={styles.deleteModalOptionsBox}>
              <div
      ...
      {tabSelected === "usuarios" ? (
        ...
        <>
          ...
          <div id="wrapBox" className={styles.wrapBox}>
            ...
            {cursosArrFilter.map((curso, i) => {
              return (
                <div key={curso._id || i} className={styles.wrapItem}>
                  {curso.name}
                  <svg
                    onClick={() => {
                      setCursoDeleteIndex(i);
                    }}
                    width="32px"
                    height="32px"
                    viewBox="0 0 24 24"
